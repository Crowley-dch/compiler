...
class CodegenTestRunner:
    ...
    def assemble_and_run(self, asm: str, test_file: Path) -> tuple:
        ...
        result = subprocess.run(['gcc', '-m64', '-mconsole', '-no-pie', '-o', str(exe_file), str(obj_file)], capture_output=True)
        if result.returncode != 0:
            return False, f"NASM failed: {result.stderr.decode()}"

        result = subprocess.run(['gcc', '-m64', '-no-pie', '-o', str(exe_file), str(obj_file)], capture_output=True)
        if result.returncode != 0:
    ...
    def run_all_tests(self):
        ...
        for test_file in sorted(self.valid_dir.rglob('*.src')):
            ...
            if success:
                print(f"  ✅ {test_file.relative_to(self.valid_dir)}: returned {result}")
                passed += 1
            else:
                print(f"  ❌ {test_file.relative_to(self.valid_dir)}: {result}")
